...
model = pickle.load(open("IMPACT_C.pkl", "rb"))
tensor_hl = np.array(model.predict(tensor_ll[f]).tolist())

tensor_ll['shift_low_level'] = (tensor_ll['shift'] -205.3726)/-0.9016
tensor_ll['IMPACT_shift'] = (tensor_hl -187.136)/-1.0399

result_df = tensor_ll[['molecule_name', 'atom_index', 'typeint', 'conn', 'x', 'y', 'z','shift_low_level', 'IMPACT_shift']]
result_df.to_csv('IMPACT_prediction_CHESHIRE_test.csv')
